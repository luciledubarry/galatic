...
name(planet)[missing_var > 0]
```

```{r}
table_planet <- planet |>
  summarise(row, col, var_missing_value)
knitr::kable(table_planet, col.names = c("Number of observations","Number of variables", "Number of observations with at least one missing value"))
```
I did not manage to display the number of missing values for each variable with missing values. 
